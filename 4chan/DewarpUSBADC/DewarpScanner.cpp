...
int main()
{
	...
	const int sampleSize = 8;		//channels times bytes per sample
	...
	{
		sprintf(buf, "..//warpedFrame_4chan_1-6656.bin");	//2048x2048 test 
		sprintf(buf, "..//warpedFrame_4chan.bin");	//2048x2048 test 
		pFile = fopen(buf, "rb");
		if (pFile == NULL)
		{
	...
	//%calculate the cutoff frequency assuming that the image is Nyquist sampled at samples / 4 (center FOV)
	double oversample[outputsamples];
	for (int i = 0; i < outputsamples - 1; i++)
		oversample[i] = index_table[i + 1] - index_table[i];

	//the difference returns 1 less value since it is calculated between each point
	//rather than interpolate/extrapolate, just mirror the last value to the first and then duplicate it
	oversample[0] = oversample[outputsamples - 2];
	...
	//we have taps and cutoffs in terms of output pixels, but will be filtering the raw data
	...
	for (int i = (int) ii; i < sampleClockLength / 2 + 1; i++)	//we got to <= and address i-1 to match the matlab code, might not be ideal
	{
		...
		//TODO:  could also skip filter coefficients for unused samples here
		...
		else
		{
			...
			//change taps in multiplies of 4 so we can pack them into common length vectors
			if (i % 4 == 1)
			{
				taps_samp[i - 1] = outtaps[tablepos];
			}
			...
			{
				//safe because we always leave 3 samples on the edge
				taps_samp[i - 1] = taps_samp[i - 2];
			}
	...
	float* btmp1 = (float*)malloc((taps_samp[0] + 1) * sizeof(float));
	float* btmp2 = (float*)malloc((taps_samp[0] + 1) * sizeof(float));
	float* btmp3 = (float*)malloc((taps_samp[0] + 1) * sizeof(float));
	float* btmp4 = (float*)malloc((taps_samp[0] + 1) * sizeof(float));

	// calculate the filter coefficients 4 at a time
	for (int i = 0; i < samples / 2; i += 4)	//note that sampleClockLength >= samples so not all of these are used
	{
		...
		{
			//calculate 4 sets of filter taps each with the same length and then interleave them so that we can load + permute taps during processing
			int taps = taps_samp[i];
			wsfiltgen(btmp1, taps_samp[i] + 2, cutoff_samp[i]);	//N+2 to match matlab fir1, there are actually N+1 total taps returned

			//sanity check 1
			if (taps_samp[i + 1] != taps)
			{
				printf("Error: invalid number of taps 1! (%d\%d)\n", taps_samp[i + 1], taps);
				return -1;
			}
			wsfiltgen(btmp2, taps_samp[i + 1] + 2, cutoff_samp[i + 1]);	//N+2 to match matlab fir1, there are actually N+1 total taps returned

			//sanity check 2
			if (taps_samp[i + 2] != taps)
			{
				printf("Error: invalid number of taps 2! (%d\%d)\n", taps_samp[i + 2], taps);
				return -1;
			}
			wsfiltgen(btmp3, taps_samp[i + 2] + 2, cutoff_samp[i + 2]);	//N+2 to match matlab fir1, there are actually N+1 total taps returned

			//sanity check 3
			if (taps_samp[i + 3] != taps)
			{
				printf("Error: invalid number of taps 2! (%d\%d)\n", taps_samp[i + 3], taps);
				return -1;
			}
			wsfiltgen(btmp4, taps_samp[i + 3] + 2, cutoff_samp[i + 3]);	//N+2 to match matlab fir1, there are actually N+1 total taps returned

			//now write out the interleaved tap coefficients so we can do 128 bit loads
			for (int ii = 0; ii < taps_samp[i] + 1; ii++)
			{
				barray[bpos++] = btmp1[ii];
				barray[bpos++] = btmp2[ii];
				barray[bpos++] = btmp3[ii];
				barray[bpos++] = btmp4[ii];
			}
	...
	for (int averages = 0; averages < timesToAverage; averages++)	//only used to average for better benchmarks
	{
		...
		for (int i = galvoResetLines; i < lines; i++)			//slow axis loop
		{
			...
			if (useFirPreFiltering)
			{
				...
				for (int ii = 0; ii < samples / 2; ii += 4)	//4 samples at a time in x4 mode to fill a 16 element YMM register
				{

					__m256i* startPtr = (__m256i*) (linedata + (ii - taps_samp[ii] / 2) * sampleSize);
					//if(ii==0)printf("%p %p %d currentTaps: %d\n", linedata, startPtr, (ii - taps_samp[ii] / 2)* sampleSize, taps_samp[ii]);

					fir_x4_avx256_fp16_opt(taps_samp[ii], barrayh + count, startPtr, (__m256h*)filtered_data + ii / 4, num_bits, dc_offset);
					count += (taps_samp[ii] + 1) * 4;
			...
			for (int j = 0; j < outputsamples; j += sampleStepSize)
			{
				...
				float frac, frac2, frac3, frac4;
				__m256i tosavearray[sampleStepSize * 2];
				...
				for (int kk = 0; kk < sampleStepSize; kk += 4)
				{
					...
					int pos = floor(index_table[j + kk]);
					int pos2 = floor(index_table[j + kk + 1]);
					int pos3 = floor(index_table[j + kk + 2]);
					int pos4 = floor(index_table[j + kk + 3]);

					frac = index_table[j + kk] - floor(index_table[j + kk]);
					frac2 = index_table[j + kk + 1] - floor(index_table[j + kk + 1]);
					frac3 = index_table[j + kk + 2] - floor(index_table[j + kk + 2]);
					frac4 = index_table[j + kk + 3] - floor(index_table[j + kk + 3]);



					__m256h frac_scalar_h = _mm256_set_ph(frac, frac, frac, frac, frac2, frac2, frac2, frac2, frac3, frac3, frac3, frac3, frac4, frac4, frac4, frac4);
					...
					if (usefp16)
					{
						if (useAVX512)
						{
							...
							//try filling filtered_data here as we get to points for locality and skipping of unused points

							
							if (useFirInlineFiltering) {

								//TODO:  could also skip filter coefficients for unused samples

								//skip unused points
								/*while (currentFilterPos < pos - 3)
								{
									count += (taps_samp[currentFilterPos] + 1)*4;
									currentFilterPos+=4;
								}*/

								for (; currentFilterPos <= pos4; currentFilterPos += 4)
								{
									int currentTaps = taps_samp[currentFilterPos];
									__m256i* startPtr = (__m256i*) (linedata + (currentFilterPos - currentTaps / 2) * sampleSize);


									fir_x4_avx256_fp16_opt(currentTaps, &barrayh[count], startPtr, (__m256h*)&(filtered_data_ptr[currentFilterPos * sampleSize]), num_bits, dc_offset);
									count += (currentTaps + 1) * 4;	//4 per pass

								}
							}


							//contains 4 channels for tap zero, then 4 channels for tap one, ...

							int tmp = (pos - 3) * sampleSize;
							data[0] = _mm256_loadu_ph((float*)&(filtered_data_ptr[tmp]));

							tmp = (pos2 - 3) * sampleSize;
							data[1] = _mm256_loadu_ph((float*)&(filtered_data_ptr[tmp]));

							tmp = (pos3 - 3) * sampleSize;
							data[2] = _mm256_loadu_ph((float*)&(filtered_data_ptr[tmp]));

							tmp = (pos4 - 3) * sampleSize;
							data[3] = _mm256_loadu_ph((float*)&(filtered_data_ptr[tmp]));


							// permute these 4 pixels (0->3) of each [A1B1C1D1 A2B2C2D2 A3B3C3D3 A4B4C4D4]_N into [A1B1C1D1_0 A1B1C1D1_1 A1B1C1D1_2 A1B1C1D1_3]
							deinterleave4x4(data, data);
					...
					cubicHermitePolyFp16(data, &tosavearray[(kk / 4)], frac_scalar_h, !useFirInlineFiltering && !useFirPreFiltering);
			...
			//finished 4 sets of 4 interleaved 4 channel pixels, now deinterleave them into 4 individual buffers outstride apart in memory
			deinterleave4x4x4(tosavearray, (output + outsamps), outstride);
		...
		if (useFirPreFiltering)
		{
			...
			for (int ii = 0; ii < samples / 2; ii += 4)	//I think we don't need to filter past samples/2 since the interpolation will never access points beyond thisuyh
			{
				...
				fir_x4_avx256_fp16_opt(taps_samp[ii], barrayh + count, (__m256i*)startPtr, (__m256h*)filtered_data + ii / 4, num_bits, dc_offset);
		...
		for (int j = outputsamples - 1; j >= 0; j -= sampleStepSize)
		{
			...
			for (int kk = 0; kk < sampleStepSize; kk += 4)
			{
				...
				int pos = (int)floor(index_table[outputsamples + j - kk]) - sampleClockLength / 2;
				int pos2 = (int)floor(index_table[outputsamples + j - kk - 1]) - sampleClockLength / 2;
				int pos3 = (int)floor(index_table[outputsamples + j - kk - 2]) - sampleClockLength / 2;
				int pos4 = (int)floor(index_table[outputsamples + j - kk - 3]) - sampleClockLength / 2;


				frac = index_table[outputsamples + j - kk] - (float)floor(index_table[outputsamples + j - kk]);
				frac2 = index_table[outputsamples + j - kk - 1] - (float)floor(index_table[outputsamples + j - kk - 1]);
				frac3 = index_table[outputsamples + j - kk - 2] - (float)floor(index_table[outputsamples + j - kk - 2]);
				frac4 = index_table[outputsamples + j - kk - 3] - (float)floor(index_table[outputsamples + j - kk - 3]);
				...
				if (usefp16)
				{
					if (useAVX512)
					{
						...
						if (useFirInlineFiltering) {
							...
							//-6 because the interpolator is 3 behind and the 4-step can be up to 3 behind as well
							for (; currentFilterPos >= pos4 - 6; currentFilterPos -= 4)
							{

								//printf("Processing %d-> %d\n", currentFilterPos, currentFilterPos + 3);
								int currentTaps = taps_samp[currentFilterPos];
								count -= (currentTaps + 1) * 4;
				...
				//tosavearray now contains 32 bit integers arranged in i256 [ABCDEFGH_0][IJKLMNOP_0][ABCDEFGH_1][IJKLMNOP_1]..., 
			}
